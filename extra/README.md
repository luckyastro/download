# Python3.14 required RPMS
Download [python3.14.deps.tar.gz (6.3 MB)](https://github.com/luckyastro/download/tree/main/extra/python3.14.deps.tar.gz)

# /usr/local/bin
Download [tar.gz (470KB)](https://github.com/luckyastro/download/tree/main/extra/user.local.bin.tar.gz)

# Postgresql 16, 17 (missing rpms)
Download [pgsql.tar.gz (1.6MB)](https://github.com/luckyastro/download/tree/main/extra/pgsql.tar.gz)

# Redis
Download [redis.tar.gz (4MB)](https://github.com/luckyastro/download/tree/main/extra/redis.tar.gz)

Download [redis.required.tar.gz (19MB)](https://github.com/luckyastro/download/tree/main/extra/redis.required.tar.gz)

# Other required RPMS for CentOS
Download [required-more.tar.gz (105MB)](https://drive.google.com/file/d/1O1Y9r_XawOsb9hcGQ-dCe3NxetfJ_9XG/view?usp=sharing)
